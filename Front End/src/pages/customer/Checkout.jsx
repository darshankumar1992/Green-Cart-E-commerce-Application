...
function Checkout() {
    ...
    //2
    useEffect(() => {
        const quantity = checkoutDetails.reduce((total, item) => total + item.quantity, 0);
        setTotalQuantity(quantity);

        const totalCheckoutAmount = checkoutDetails.reduce((total, item) => {
            const { quantity, productItem } = item;
            const { price, discount } = productItem;
            return total + quantity * calculateDiscount(price, discount);
        }, 0);

        setCheckoutAmount(totalCheckoutAmount);
    }, [checkoutDetails]);
    

    //3 tax
    useEffect(() => {
        const tax = checkoutAmount * 0.02;
        setTaxAmount(tax);
        setTotalAmount(checkoutAmount + tax);
    }, [checkoutAmount]);

    const handlePlaceOrderClick = () => {
        if(!checkoutDetails || checkoutDetails.length === 0){
            toast.error("CheckOut Data is Empty")
            return;
        }
        if (!deliveryAddress) {
    ...
    const handleCancelItem = (itemIdToRemove) => {
        const updatedCheckoutDetails = checkoutDetails.filter(item => item.productItem.itemId !== itemIdToRemove);
        toast.success("Item Removed")
        setCheckoutDetails(updatedCheckoutDetails);
    };

    const handleIncrementQuantity = (itemId) => {
        const updatedDetails = checkoutDetails.map(item => {
            ...
            <div className="flex flex-col md:flex-row py-16 max-w-6xl w-full mx-auto">
                <div className="flex-1 max-w-4xl">
                    <h1 className="text-3xl font-medium mb-6 text-gray-700">
                        Checkout Details <span className="text-sm text-[#4FBF8B]">{totalQuantity} Items</span>
                    </h1>
                    ...
                    {checkoutDetails.map((item, index) => {
                        ...
                        return (
                            ...
                            <button className="cursor-pointer mx-auto" onClick={() => handleCancelItem(product.itemId)}>
                ...
                <div className="max-w-[360px] w-full bg-gray-100/90 p-5 max-md:mt-16 border border-gray-300/70">
                    ...
                    <div className="text-gray-500 mt-4 space-y-2">
                        <p className="flex justify-between">
                            <span>Price</span><span> {checkoutAmount}</span>
                        </p>
                        <p className="flex justify-between">
